...
//image[c100][実運用（画像は@<href>{https://twitter.com/sugerest/status/1558255201098027008}より][scale=0.3]
//}
「精算機作るよ！」の概要としては、
...
精算機を構成する代表的なコンポーネントには下記が含まれ、これらは分離された状態で搬入されます。

 * コインメック
 * ビルバリ
 * バーコードリーダー（QRコード決済用）
 * Square Terminal（クレジットカード・電子マネー決済用）およびEthernet接続用の専用ハブとACアダプタ
...
具体的な改良点は下記で述べますが、内容としては
組み立てた状態で搬入すべくダンボールに入る程度に小型化する、
またコンポーネント自体を減らすためにUSB Gadget Driverを@<b>{試そうとした}記録です。
...
お釣りの取り出し口として確保していた空間を狭めたことで実現は出来ましたが、
...
求める要件としては下記のとおりであり、正直これならRaspberry Pi 4でなくとも満たす製品はあります。
...
また、Raspberry Pi 4ではありがたいことに有志のおかげで様々な情報を得ることができますが、それ以外だとあまり期待できません。
ということで、Raspberry Pi 4を用いました。
最近は手に入れやすくなり良かったです。

さらに、同時のACアダプタや配線ケーブルの余りも裏のデッドスペースに収めることにします。
以上の対応で運搬しやすく、またデッドスペースを@<img>{cable}のように活用し、すっきり@<fn>{sukkiri}と収めることができました。
//image[cable][デッドスペースの活用][scale=0.3]
...
2000円札自体が手元になく検証が出来ない、結果として2000円札は受け入れられない状態になっていました。

出来れば対応したいですよね？ということで、まずは検証用2000円札の入手から始めます。
...
//image[2000yen][勝ちまくりモテまくり][scale=0.5]
...
精算機で利用しているバーコードリーダーはデンソーのQK30で、これは二度読み防止機能が備わっています。
...
//image[paypay][PayPayバーコード支払い画面][scale=0.2]
...
ただ、PayPayのきせかえデザインによっては待ち受け画面には1次元バーコードしか出さないものもあり、これだとわざわざ支払画面に飛んでもらう必要があります。
じゃあ1次元バーコードしか読み込まないようにすれば？というのはその通りですが、せっかくQRコードも読めるリーダーを使っており、
また1次元バーコードかQRコードかどちらか早く読み込めた情報を使えば決済を早く出来たりという期待を込めて、
バーコード種別が混在していても二度読みが防止できるようにしたいです。
...
//image[offline][PayPayオフライン決済を試した際のウォレット画面][scale=0.2]
...
何らかの決済結果の通知が必要に必要になりますが、QK30では外部からブザーやLEDを制御できます。
自由に制御できるわけではなく数パターンの中から選ぶ必要はありますが、十分です。
もっと言えばスピーカーでWAVファイルも再生できるので本当に「PayPay♪」と喋らせることも言えますが…別にいいかなと思います。
...
: CONFIRMATION, SIGNATURE, DATA_COLLECTION, SELECT, QR_CODE
  順に確認画面、署名画面、顧客情報の収集画面、選択ボタン表示画面、QRコードの表示画面に切り替えることができる。
  それぞれの画面遷移も設定できる。
...
//image[powerdbysquare][Powerd By Square][scale=0.4]
//image[sqimg][カスタム画像][scale=0.4]
//}
...
povoにトラブルが起きると使えなくなるわけで、下記で対策案を検討しました。
...
: 公衆無線LANの利用
  ...
  d Wi-Fiであれば認証にEAP-PEAPが利用でき、自動接続が出来るようですが未検証のため見送りました。
: プライベートLoRaの利用
  LoRAではありません。
...
詳細はURL先の動画で確認したほうが早いですが、文字にして残すと下記の手順で転写を行っています。
...
2回目（@<img>{onetwo}右）は最悪という状況でしたが、
3回目（@<img>{san}）は割りとうまくいきました。
3回目もOHPフィルムが熱で反ったことで若干波打ったりかすれたりはありますが、許容範囲でしょう。
//subfig[熱転写を試した結果]{
//image[onetwo][1回目（左）と2回目（右）][scale=0.4]
//image[san][3回目（精算機全体込み）][scale=0.4]
//}
...
配送センター間は混載輸送が出来るかも知れませんが、やはりそのぐらいしますよね。
